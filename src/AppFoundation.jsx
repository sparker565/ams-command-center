...
function AppFoundation() {
  ...
  useEffect(() => {
    ...
    const savedUser = appState.users.find(
      (u) => u.id === appState.ui.currentUserId
    );
    if (!savedUser) return;
  ...
  const vendorSites = currentUser
    ...
    : [];
  const currentVendorRecord = currentUser?.vendorId
    ? normalizedVendors.find((vendor) => vendor.id === currentUser.vendorId)
    : null;
  const availableVendorWork =
    currentUser && currentUser.role === ROLES.VENDOR
      ? appState.workOrders.filter((workOrder) => {
          ...
          return currentVendorRecord.serviceTypes.includes(
  ...
  const activeJobs = appState.jobs.filter(
    (entry) => entry.status !== "Completed"
  );
