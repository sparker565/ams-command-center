...
function getCanonicalSellValue(data) {
  return String(data?.sell ?? data?.sellPrice ?? "").trim();
}

export function normalizeFirestoreJob(docSnapshot) {
  const data = docSnapshot.data() || {};
  const title = data.title || data.siteName || "Firestore Job";
  const normalizedSell = getCanonicalSellValue(data);
  ...
  return {
    ...
    servicePerformed: normalizeServiceType(data.servicePerformed || data.serviceType) || "",
...
function serializeJobCreate(job) {
  return {
    ...
    sell: getCanonicalSellValue(job),
    serviceType: normalizeServiceType(job.serviceType) || "",
    state: job.state || "",
...
export function buildFirestoreJobPayload({ workOrder, vendor, price, sell, createdBy }) {
  ...
  const serviceType = normalizeServiceType(workOrder.serviceType) || "";
