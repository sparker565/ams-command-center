...
const ensureOwnerUser = (users = []) => {
  const existingOwnerIndex = users.findIndex((u) => u.role === ROLES.OWNER);

  const lockedOwner = {
    id:
      existingOwnerIndex >= 0 && users[existingOwnerIndex]?.id
        ? users[existingOwnerIndex].id
        : createId("user"),
    name: "Sparker",
    email: "Sparker",
    password: "Tyson5655",
  ...
  return [lockedOwner, ...users];
};
function normalizeStateData(state) {
  const users = ensureOwnerUser(state.users || []);
  ...
  const selectedSiteId =
    ...
    (state.sites || []).some((site) => site.id === state.ui.selectedSiteId)
      ? state.ui.selectedSiteId
...
function getLatestVendorProposal(proposals, workOrderId, vendorId) {
  return (
    getVendorWorkOrderProposals(proposals, workOrderId, vendorId)[0] || null
  );
}

function isVendorEligibleForWorkOrder({
  workOrder,
  site,
  vendor,
  proposals,
  ...
  if (!["opportunity", "under_review"].includes(workOrder.proposalState))
    return false;
...
function canVendorSubmitProposal({ workOrder, site, vendor, proposals, jobs }) {
  if (
    !isVendorEligibleForWorkOrder({ workOrder, site, vendor, proposals, jobs })
  )
    return false;
...
function AppBuild03() {
  ...
  useEffect(() => {
    ...
    const savedUser = appState.users.find(
      (user) => user.id === appState.ui.currentUserId
    );
    if (!savedUser) return;
  ...
  const removeSite = (siteId) => {
    ...
    updateAppState((current) => ({
      ...
      workOrders: current.workOrders.filter(
        (workOrder) => workOrder.siteId !== siteId
      ),
  ...
  const saveUser = () => {
    ...
    if (editingUserId) {
      ...
      updateAppState((current) => ({
        ...current,
        users: [
          { id: createId("user"), active: true, ...userForm },
          ...current.users,
        ],
      }));
    }
  ...
  const assignVendorToWorkOrder = (workOrderId) => {
    ...
    if (workOrder.proposalRequired) {
      window.alert(
        "Proposal-based work orders must be awarded through the proposal review workflow."
      );
      return;
    }
  ...
  const openWorkOrders = appState.workOrders.filter(
  ...
  useEffect(() => {
    ...
    setReviewForm({
      reviewedPrice:
        selectedProposal.reviewedPrice || selectedProposal.submittedPrice || "",
  ...
  const handleApproveProposal = () => {
    ...
    const vendor = normalizedVendors.find(
      (entry) => entry.id === selectedProposal.vendorId
    );
    if (!vendor) {
    ...
    const existingJob = appState.jobs.find(
      (job) => job.workOrderId === selectedWorkOrder.id
    );
    if (existingJob) {
    ...
    const approvedPrice =
      ...
      selectedProposal.reviewedPrice ||
      selectedProposal.submittedPrice;
    const newJob = buildJobRecord({
  ...
  const vendorProposalHistory =
    currentVendorRecord && currentUser?.role === ROLES.VENDOR
      ? sortByNewest(
          appState.proposals.filter(
            (proposal) => proposal.vendorId === currentVendorRecord.id
          ),
          "submittedAt"
        )
      : [];
    ...
    <div className="screen-grid">
      ...
      >
        <p className="supporting-copy">
          Full visibility is active for the additive 0.3 build. Proposal
          workflow review is available through the work order screen without
          changing the existing command layout.
        </p>
      </PageSection>
    </div>
  );
    ...
    <PageSection title="My Jobs">
      {visibleVendorJobs.length ? (
        ...
        <EmptyState
          ...
          text="Assigned jobs will appear here."
        />
      )}
    </PageSection>
  );
    ...
    <PageSection title="My Sites">
      {vendorSites.length ? (
        ...
        <EmptyState
          ...
          text="Assigned site information will appear here."
        />
      )}
    </PageSection>
  );
        ...
        <div className="available-work-grid">
          {availableVendorWork.map((workOrder) => {
            ...
            const latestProposal = currentVendorRecord
              ? getLatestVendorProposal(
                  appState.proposals,
                  workOrder.id,
                  currentVendorRecord.id
                )
              : null;
            const actionLabel = latestProposal
              ? "Resubmit Proposal"
              : "Submit Proposal";
            ...
            return (
              <article key={proposal.id} className="proposal-history-card">
                ...
                <div className="proposal-history-meta">
                  <span>
                    {proposal.requestedRevisionAt
                      ? `Revision requested ${formatDate(
                          proposal.requestedRevisionAt
                        )}`
                      : null}
                  </span>
                  ...
                  <span>
                    {proposal.approvedAt
                      ? `Approved ${formatDate(proposal.approvedAt)}`
                      : null}
                  </span>
                </div>
                {canResubmit ? (
            ...
            },
            {
              ...
              render: (row) => (
                <div className="table-actions">
                  ...
                  <button
                    className="secondary-button"
                    onClick={() => toggleUserActive(row.id)}
                  >
        ...
        <DataTable
          columns={[
            ...
            { key: "address", label: "Address", render: (row) => row.address },
            {
              key: "state",
              label: "State",
              render: (row) => row.state || "NA",
            },
            {
              key: "notes",
              label: "Internal Notes",
              render: (row) => row.internalNotes || "None",
            },
            {
              key: "actions",
              label: "Actions",
              render: (row) => (
                <div className="table-actions">
                  <button
                    className="secondary-button"
                    onClick={() => startEditSite(row)}
                  >
                    Edit
                  </button>
                  <button
                    className="secondary-button"
                    onClick={() => removeSite(row.id)}
                  >
                    Remove
                  </button>
                </div>
            ...
            },
            {
              ...
              render: (row) => (
                <div className="table-actions">
                  <button
                    className="secondary-button"
                    onClick={() => startEditVendor(row)}
                  >
                    Edit
                  </button>
                  <button
                    className="secondary-button"
                    onClick={() => toggleVendorActive(row.id)}
                  >
                    {row.active ? "Deactivate" : "Activate"}
                  </button>
                </div>
        ...
        <DataTable
          columns={[
            ...
            },
            {
              ...
              render: (row) =>
                appState.proposals.filter(
                  (proposal) => proposal.workOrderId === row.id
                ).length,
            },
            {
              key: "status",
            ...
            },
            {
              ...
              label: "Assignment",
              render: (row) => {
                const relatedJob = appState.jobs.find(
                  (job) => job.workOrderId === row.id
                );
                ...
                return (
                  <div className="assignment-cell">
                    <select
                      value={
                        jobAssignment[row.id] || row.assignedVendorId || ""
                      }
            ...
            },
            {
              ...
              render: (row) => (
                <select
                  value={row.status}
                  onChange={(event) =>
                    updateWorkOrderStatus(row.id, event.target.value)
                  }
                >
      ...
      <PageSection title="Work Order Detail">
        {!selectedWorkOrder ? (
          <EmptyState
            ...
            text="Choose a work order to review proposal details."
          />
        ) : (
                    ...
                    <div className="proposal-decision-card">
                      <div className="proposal-decision-header">
                        <div>
                          ...
                          <p>
                            ...
                            • Revision {selectedProposal.revisionCount}
                          </p>
                        </div>
                      ...
                      <div className="proposal-summary-grid">
                        ...
                        <div>
                          ...
                          <p>
                            {selectedProposal.submittedNotes ||
                              "No submitted notes."}
                          </p>
                        </div>
                      </div>
            ...
            },
            {
              ...
              render: (row) => (
                <select
                  value={row.status}
                  onChange={(event) =>
                    updateJobStatus(row.id, event.target.value)
                  }
                >
  ...
  function renderScreen() {
    ...
    if (activeScreen === "myJobs")
      return (
        <div className="screen-grid vendor-screen">{vendorJobsSection}</div>
      );
    if (activeScreen === "mySites")
      return (
        <div className="screen-grid vendor-screen">{vendorSitesSection}</div>
      );
